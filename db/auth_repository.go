...
type AuthRepository interface {
	...
	IsReferralCodeExist(referralCode string) error
...
func (a *authRepo) IsPhoneExist(phone string) error {
	var count int64
	err := a.DB.Model(&models.User{}).Where("phone = ?", phone).Count(&count).Error
	if err != nil {
		return errors.Wrap(err, "gorm.count error")
	}
	if count > 0 {
...
func (a *authRepo) VerifyEmail(email string, token string) error {
	err := a.DB.Model(&models.User{}).Where("email = ?", email).Updates(models.User{EmailConfirmed: true}).Error
	if err != nil {
		return err
	}
...
func (a *authRepo) UpdatePassword(password string, email string) error {
	err := a.DB.Model(&models.User{}).Where("email = ?", email).Updates(models.User{PasswordHash: password}).Error
	if err != nil {
		return err
	}
	return nil
...
func (a *authRepo) IsReferralCodeExist(referralCode string) error {
	var count int64
	err := a.DB.Model(&models.User{}).Where("referral_code = ?", referralCode).Count(&count).Error
	if err != nil {
		return errors.Wrap(err, "gorm.count error")
	}
	if count < 1 {
